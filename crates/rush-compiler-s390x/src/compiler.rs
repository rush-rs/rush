...
impl<'tree> Compiler<'tree> {
    ...
    fn function_definition(&mut self, node: AnalyzedFunctionDefinition<'tree>) {
                    ...
                    match FloatRegister::nth_param(float_cnt) {
                        Some(reg) => {
                            ...
                            self.scope_mut().insert(
                                ...
                                Variable {
                                    type_: param.type_,
                                    value: Some(Pointer::Register(IntRegisterPointer(
                                        IntRegister::R15,
                                        offset,
                                    ))),
                                },
                    ...
                    match IntRegister::nth_param(int_cnt) {
                        Some(reg) => {
                            ...
                            match size {
                                ...
                                Size::Double => param_store_instructions.push((
                                    Instruction::Store64(
                                        reg,
                                        IntRegisterPointer(IntRegister::R15, offset),
                                    ),
        ...
        if loads_params_from_stack {
            ...
            self.use_reg(IntRegister::R13.into(), Size::Double);
        }
    ...
    fn function_body(&mut self, node: AnalyzedBlock<'tree>) {
        ...
        if let Some(expr) = node.expr {
            ...
            match self.expression(expr) {
                ...
                Some(Register::Float(reg)) => self.insert_movf(FloatRegister::F0, reg),
    ...
    fn for_stmt(&mut self, node: AnalyzedForStmt<'tree>) {
        ...
        self.scope_mut().insert(
            ...
            Variable {
                type_,
                value: ptr.map(Pointer::Register),
            },
        );
    ...
    pub(crate) fn save_freg_on_stack(
        &mut self,
        reg: FloatRegister,
        ...
        match node {
            AnalyzedExpression::Int(value) => {
                ...
                if !(-2147483648..=2147483647).contains(&value) {
                    ...
                    self.insert_with_comment(
                        Instruction::Xgr(dest_reg, dest_reg),
                        format!("{dest_reg} = 0").into(),
                    );
            ...
            AnalyzedExpression::Float(value) => {
                ...
                self.use_reg(IntRegister::R0.into(), Size::Double);
            ...
            AnalyzedExpression::Ident(ident) => {
                ...
                let (ptr, type_) = match self.resolve_variable(ident.ident).type_ {
                    ...
                    type_ => (self.load_variable_from_name(ident.ident), type_),
                };
    ...
    fn bool_neg(&mut self, source_reg: IntRegister) -> IntRegister {
        ...
        self.insert_with_comment(
            Instruction::ShiftRightSingleLogicalImm(dest_reg, 31),
            "bool neg".into(),
        );
    ...
    fn prefix_expr(&mut self, node: AnalyzedPrefixExpr<'tree>) -> Option<Register> {
                ...
                let dest_reg = match variable.value {
                    Some(Pointer::Register(pointer)) => {
                        ...
                        self.insert_with_comment(
                            Instruction::Aghi(
                                dest_reg,
                                pointer.offset().try_into().expect("offset too large"),
                            ),
                    ...
                    Some(Pointer::Label(_)) => {
                        ...
                        self.insert_with_comment(
                            Instruction::Aghi(
                                dest_reg,
                                loaded.offset().try_into().expect("offset too large"),
                            ),
                            format!("&{}: offset", ident.ident).into(),
                        );
        ...
        match (lhs_type, node.op) {
            (Type::Int(0), PrefixOp::Neg) => {
                self.use_reg(lhs_reg, Size::Double);
                let zero_reg = self.get_int_reg();
                self.use_reg(zero_reg.into(), Size::Double);
                self.insert_with_comment(
                    Instruction::Lghi(zero_reg, 0),
                    format!("negate {lhs_reg}").into(),
                );
                let dest_reg =
                    self.infix_helper(zero_reg.into(), lhs_reg, InfixOp::Minus, Type::Int(0));
            ...
            (Type::Float(0), PrefixOp::Neg) => {
                ...
                let dest_reg =
                    self.infix_helper(float_reg, lhs_reg, InfixOp::Minus, Type::Float(0));
                self.release_reg(lhs_reg);
                Some(dest_reg)
            }
        ...
        match (node.lhs, node.rhs, node.op) {
            ...
            | (expr, AnalyzedExpression::Int(value), InfixOp::Plus) => {
                ...
                self.insert(Instruction::Aghi(
                    rhs_reg.into(),
                    value.try_into().expect("offsett too large"),
                ));
                Some(rhs_reg)
            }
    ...
    fn int_division_helper(
        ...
        const REGS_USED_FOR_DIVISION: [IntRegister; 4] =
            [EVEN_PAIR_LOW, EVEN_PAIR_HIGH, ODD_PAIR_LOW, ODD_PAIR_HIGH];
    ...
    fn float_division_helper(
        ...
        const REGS_USED_FOR_DIVISION: [FloatRegister; 2] = [EVEN_PAIR_LOW, EVEN_PAIR_HIGH];
        ...
        for (r, offset) in saved {
            if r == dest_reg {
                unreachable!("bug in compiler implementation")
            }
        ...
        let res = match (type_, op) {
            ...
            (Type::Int(0), InfixOp::Shl) => {
                ...
                if base_reg == IntRegister::R0 {
                    self.use_reg(dest_regi.into(), Size::Double);
                    self.use_reg(lhs, Size::Double);
            ...
            (
                ...
                self.insert_with_comment(
                    Instruction::Lghi(dest_regi, false as i16),
                    "`false` case of comp".into(),
                );
                ...
                self.insert_with_comment(
                    Instruction::Lghi(dest_regi, true as i16),
                    "`true` case of comp".into(),
                );
    ...
    fn assign_expr(&mut self, node: AnalyzedAssignExpr<'tree>) {
        ...
        if node.assignee_ptr_count > 0 {
            ...
            self.use_reg(ptr_reg.into(), Size::Double);
            ...
            // copy into ptr_reg
            if ptr_reg != p.reg() {
                self.insert_movi(ptr_reg, p.reg(), file!(), line!());
            }
    ...
    fn cast_expr(&mut self, node: AnalyzedCastExpr<'tree>) -> Option<Register> {
        ...
        let res = match (lhs_type, node.type_) {
            ...
            (Type::Float(0), Type::Bool(0)) => {
                ...
                self.infix_helper(zero_reg, lhs_reg, InfixOp::Neq, Type::Float(0))
            }
            _ => unreachable!(
                "cannot use other combinations in a typecast: {lhs_type} {}",
                node.type_
            ),
        };
    ...
    fn if_expr(&mut self, node: AnalyzedIfExpr<'tree>) -> Option<Register> {
        ...
        self.insert(Instruction::BranchEq(if node.else_block.is_some() {
