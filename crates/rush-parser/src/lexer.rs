...
mod tests {
    ...
    fn single_tokens() {
        let tests = [
            ...
            ("loop", Ok(TokenKind::Loop.spanned(span!(0..4)))),
            ("while", Ok(TokenKind::While.spanned(span!(0..5)))),
            ("for", Ok(TokenKind::For.spanned(span!(0..3)))),
            ("break", Ok(TokenKind::Break.spanned(span!(0..5)))),
            ("continue", Ok(TokenKind::Continue.spanned(span!(0..8)))),
