...
impl<'tree> Compiler<'tree> {
    ...
    fn declare_main_fn(
        &mut self,
        node: AnalyzedBlock<'tree>,
        globals: Vec<AnalyzedLetStmt<'tree>>,
    ) {
        // create `_start` label
        let start_label = "_start";
        self.blocks.push(Block::new(start_label.into()));
        let fn_label = "main..main";
        self.blocks.push(Block::new(fn_label.into()));
        self.exports.push("_start".into());

        // call the main function
        self.insert(Instruction::Call(fn_label.into()));
        ...
        self.insert_at(fn_label); // resets the current block back to the fn block
    ...
    fn statement(&mut self, node: AnalyzedStatement<'tree>) {
        match node {
            ...
            AnalyzedStatement::Break => {
                self.insert(Instruction::Comment("break".into()));
                self.insert_jmp(Rc::clone(&self.curr_loop().after_loop))
            }
            AnalyzedStatement::Continue => {
                self.insert(Instruction::Comment("continue".into()));
                self.insert_jmp(Rc::clone(&self.curr_loop().loop_head))
            }
    ...
    fn return_stmt(&mut self, node: AnalyzedReturnStmt<'tree>) {
        ...
        self.insert_jmp(Rc::clone(&self.curr_fn().epilogue_label));
    }
    ...
    fn load_value_from_variable(&mut self, var: Variable, ident: &'tree str) -> Option<Register> {
        match var.value {
